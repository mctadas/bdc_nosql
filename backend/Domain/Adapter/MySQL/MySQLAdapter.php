...
use Domain\Adapter\MySQL\Command\Select as SelectCommand;
...
class MySQLAdapter extends Adapter 
{
	...
	public function proceed() 
	{
		...
		$message = $this->_di->mysqlQueue->deque();
		...
		try {
			switch ($message['class_name']) {
				...
				case 'Domain\Adapter\MySQL\Command\Select':

					$result = $this->select($message);
					break;

				case 'Domain\Adapter\MySQL\Command\Export':
		...
		if ($success) {
			$this->_di->mysqlQueue->removeFromQueue($message["identity"]);
		} else {
			$this->_di->mysqlQueue->updateFailedQueue($message['identity']);
	...
	public function export($data) 
	{
		...
		try {
			...
			$remote->query('SET NAMES utf8');
			$remote->query('SET CHARACTER SET utf8');
		} catch (\PDOException $ex) {

			return array('error' => 'apiErrors.mysql.unableToConnect');
		}
		...
		foreach ($params['datamap'] as $localColumn => $remoteColumn) {
			...
			if (!isset($columns[$localColumn])) {
				...
				return array('error' => 'apiErrors.mysql.unknownLocalColumn');
			}
		...
		for ($i = 1; $i <= $iterations; $i++) {
			...
			foreach ($localResults as $resultRow) {
				...
				if ($rowVO === false && $resultRow['data'] != 'b:0;') {
					...
					return array('error' => 'apiErrors.mysql.unableToUnserialize');
				...
				try {
					...
					return array('error' => 'apiErrors.mysql.unableToInsert');
				}
			}
	...
	public function select(&$data) 
	{
		...
		if (!isset($params['from'])) {
			...
			$stmt = $dbh->query('DESCRIBE `' . $params['table'] . '`');

			if (empty($stmt) || !$stmt instanceof \PDOStatement) {

				return array('error' => 'apiErrors.mysql.unknownTable');
			}
			...
			foreach ($columns as $column) {
				...
				if (!empty($params[$column['Field']]) && in_array($column['Type'], array('text', 'mediumtext', 'longtext', 'blob', 'mediumblob', 'longblob'))) {
			...
			foreach ($params['columns'] as $column => $column_type) {
				...
				if (!in_array($column, $available, true)) {
					...
					return array('error' => 'apiErrors.mysql.unknownColumn');
				}
			}
		...
		$stmt = $dbh->query('SELECT ' . $columns . ' FROM `' . $params['table'] . '` LIMIT ' . $params['from'] . ', ' . $select_rows);

		if (empty($stmt) || !$stmt instanceof \PDOStatement) {

			return array('error' => 'apiErrors.mysql.unableToSelect');
		}
	 ...
	 * @return \Domain\Adapter\MySQL\successEvent 
	 */
	protected function _buildEvent($message, array $result, &$success) 
	{
		switch ($message['class_name']) {
			...
			case 'Domain\Adapter\MySQL\Command\Select':
				...
				$successEvent = 'Domain\Adapter\MySQL\Event\Selected';
				$failedEvent = 'Domain\Adapter\MySQL\Event\SelectionFailed';
				$resultVO = 'Domain\Adapter\MySQL\ValueObject\RowsList';

				break;

			case 'Domain\Adapter\MySQL\Command\Export':

				$successEvent = 'Domain\Adapter\MySQL\Event\Exported';
				$failedEvent = 'Domain\Adapter\MySQL\Event\ExportFailed';
