...
class Query extends Command 
{
	...
	static public function buildFromRequestData(array $requestData) 
	{
		...
		if (empty($requestData['attributes']['server']) || (!preg_match('/htt(p|ps):\/\//', $requestData['attributes']['server']) )) {

			$validationErrors['attributes'] = 'validationErrors.attributes.invalidServerURI';
		}
		...
		if (!is_array($requestData['attributes']['columns'])) {

			$validationErrors['attributes']['columns'] = 'validationErrors.attributes.columns.emptyColumns';
		} else {

			foreach ($requestData['attributes']['columns'] as $column => $type) {

				if (!in_array($type, $validColumns, true)) {

					$validationErrors['attributes']['columns'] = 'validationErrors.attributes.columns.invalidColumnType';
					break;
				}
			}
		}
