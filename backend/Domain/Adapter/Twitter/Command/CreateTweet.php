...
namespace Domain\Adapter\Twitter\Command;
 ...
 * @package Kompro 
 */
class CreateTweet extends Command 
{
	...
	static public function buildFromRequestData(array $requestData) 
	{
		...
		if (empty($requestData['attributes']['access_token'])) {
			...
			$validationErrors['attributes']['access_token'] = 'validationErrors.attributes.access_token.missingAccessToken';
		}

		if (empty($requestData['attributes']['access_token_secret'])) {

			$validationErrors['attributes']['access_token_secret'] = 'validationErrors.attributes.access_token_secret.missingAccessTokenSecret';
		}
